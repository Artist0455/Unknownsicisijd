...
def get_recent_users_buttons_with_message(user_id, message_text):
    """Get recent users buttons with pre-filled message"""
    try:
        if not recent_users:
            return []
        ...
        for user_key, user_data in sorted_users[:5]:
            ...
            if username:
                display_text = f"@{username}"
                switch_query = f"{message_text} @{username}"
            else:
                display_text = f"{first_name}"
                switch_query = f"{message_text} {first_name}"
        ...
        logger.error(f"Error getting recent users with message: {e}")
        return []

def parse_whisper_message(text):
    """Smart parsing of whisper message with INSTANT detection"""
    ...
    # 🔥 ULTRA-IMPROVED: Multiple flexible patterns for INSTANT detection
    
    # Pattern 1: @username at the end (most common) - "hello @username"
    pattern1 = r'^(.*?)\s+@(\w+)$'
    match1 = re.match(pattern1, text)
    if match1:
        message = match1.group(1).strip()
        target = match1.group(2).strip()
        if message and target:
            logger.info(f"✅ Pattern1 matched: Message='{message}', Target='{target}'")
            return message, target
    
    # Pattern 2: User ID at the end - "hello 123456789"
    pattern2 = r'^(.*?)\s+(\d{5,15})$'
    match2 = re.match(pattern2, text)
    if match2:
        message = match2.group(1).strip()
        target = match2.group(2).strip()
        if message and target:
            logger.info(f"✅ Pattern2 matched: Message='{message}', Target='{target}'")
            return message, target
    
    # Pattern 3: @username immediately after text - "hello@username"
    pattern3 = r'^(.+?)@(\w+)$'
    match3 = re.match(pattern3, text)
    if match3:
        message = match3.group(1).strip()
        target = match3.group(2).strip()
        if message and target:
            logger.info(f"✅ Pattern3 matched: Message='{message}', Target='{target}'")
            return message, target
    
    # Pattern 4: Only @username - "@username"
    pattern4 = r'^@(\w+)$'
    match4 = re.match(pattern4, text)
    if match4:
        target = match4.group(1).strip()
        logger.info(f"✅ Pattern4 matched: Target='{target}'")
        return "🔒 You have a secret message!", target
    
    # Pattern 5: Only user ID - "123456789"
    pattern5 = r'^(\d{5,15})$'
    match5 = re.match(pattern5, text)
    if match5:
        target = match5.group(1).strip()
        logger.info(f"✅ Pattern5 matched: Target='{target}'")
        return "🔒 You have a secret message!", target
    
    # Pattern 6: @username in middle with spaces - "hello @username how are you"
    pattern6 = r'^(.+?)\s+@(\w+)(?:\s+(.+))?$'
    match6 = re.match(pattern6, text)
    if match6:
        message_part1 = match6.group(1).strip()
        target = match6.group(2).strip()
        message_part2 = match6.group(3) or ""
        message = f"{message_part1} {message_part2}".strip()
        if message and target:
            logger.info(f"✅ Pattern6 matched: Message='{message}', Target='{target}'")
            return message, target
    
    # Pattern 7: User ID in middle with spaces - "hello 123456789 how are you"
    pattern7 = r'^(.+?)\s+(\d{5,15})(?:\s+(.+))?$'
    match7 = re.match(pattern7, text)
    if match7:
        message_part1 = match7.group(1).strip()
        target = match7.group(2).strip()
        message_part2 = match7.group(3) or ""
        message = f"{message_part1} {message_part2}".strip()
        if message and target:
            logger.info(f"✅ Pattern7 matched: Message='{message}', Target='{target}'")
            return message, target
    
    logger.info(f"❌ No pattern matched for: '{text}'")
    return None, None
...
async def handle_inline_query(event, client=None):
    """Handle inline queries with INSTANT username detection"""
    if client is None:
        client = bot
    ...
    try:
        ...
        logger.info(f"📨 Inline query from {event.sender_id}: '{query_text}'")
        
        # If empty query, show recent users directly
        if not query_text:
        ...
        # 🔥 ULTRA-IMPROVED: Instant detection with smart parsing
        message_text, target_user = parse_whisper_message(query_text)
        
        # If we successfully parsed message and target INSTANTLY
        if message_text is not None and target_user is not None:
            logger.info(f"🎯 Instant detection successful: Target='{target_user}'")
        ...
        # If no target found but we have text, show recent users with the message pre-filled
        if not target_user and query_text:
            recent_buttons = get_recent_users_buttons_with_message(event.sender_id, query_text)
            if recent_buttons:
                result = event.builder.article(
                    title=f"🤫 Send: '{query_text[:30]}...'",
                    description="Select recipient from recent users",
                    text=f"**Message:** {query_text}\n\n**Select recipient:**",
                    buttons=recent_buttons
                )
                await event.answer([result])
                return
                ...
                result = event.builder.article(
                    ...
                    description="Use: message @username OR select from recent",
                    text=f"**Your message:** {query_text}\n\n**Please add @username or user ID:**\n\nExamples:\n• `{query_text} @username`\n• `{query_text} 123456789`",
...
def home():
    ...
    <body>
        <div class="container">
            ...
            <p><strong>Usage:</strong> Use inline mode in any chat: <code>@{} message @username</code></p>
        </div>
    </body>
    </html>
    """.format(
