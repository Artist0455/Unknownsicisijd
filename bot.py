...
def parse_whisper_message(text):
    """Smart parsing of whisper message with multiple patterns"""
    if not text or not text.strip():
        return None, None
    
    text = text.strip()
    
    # Pattern 1: @username at the end
    pattern1 = r'^(.*?)\s+@(\w+)$'
    ...
    # Pattern 3: @username in middle (flexible)
    pattern3 = r'^(.*?)\s+@(\w+)\s*(.*)?$'
    match3 = re.match(pattern3, text)
    if match3:
        message = match3.group(1).strip()
        target = match3.group(2).strip()
        remaining = match3.group(3) or ""
        if message and target:
            # Include remaining text in message
            if remaining:
                message = f"{message} {remaining}".strip()
            return message, target
    
    # Pattern 4: User ID in middle (flexible)
    pattern4 = r'^(.*?)\s+(\d+)\s*(.*)?$'
    match4 = re.match(pattern4, text)
    if match4:
        message = match4.group(1).strip()
        target = match4.group(2).strip()
        remaining = match4.group(3) or ""
        if message and target:
            # Include remaining text in message
            if remaining:
                message = f"{message} {remaining}".strip()
...
"""
                ...
                if event.sender_id == msg_data['user_id']:
                    # Target user opening the message - NO "From" text
...
async def handle_inline_query(event, client=None):
    """Handle inline queries with recent users suggestions"""
    if client is None:
        client = bot
    ...
    try:
        ...
        # 🔥 NEW: If empty query, show recent users directly (Like your screenshot)
        if not query_text:
            ...
            if recent_buttons:
                result = event.builder.article(
                    ...
                    text="**Select a recent user to whisper:**\n\nOr type your message in format: `message @username`",
        ...
        if not target_user and query_text:
            ...
            if recent_buttons:
                ...
                result = event.builder.article(
                    ...
                    text=f"**Your message:** {query_text}\n\n**Please add @username or user ID at the end:**\n\nExample: `{query_text} @username`",
                    buttons=[[Button.switch_inline("🔄 Try Again", query=query_text)]]
                )
                await event.answer([result])
                return

        # 🔥 NEW: If we have message but no target, guide user
        if not target_user:
            result = event.builder.article(
                title="❌ No recipient specified",
                description="Add @username or user ID at the end",
        ...
        if not message_text:
            result = event.builder.article(
                ...
                text="Please write your message before the @username\n\n**Format:** `your_message @username`",
                buttons=[[Button.switch_inline("🔄 Try Again", query=query_text)]]
            )
            await event.answer([result])
            return
