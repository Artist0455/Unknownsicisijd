...
class WhisperBot:
    ...
    def initialize_bot(self):
        ...
        try:
            ...
            self.bot = TelegramClient('whisper_bot', API_ID, API_HASH, loop=self.loop)
...
"""
        ...
        async def broadcast_handler(event):
            ...
            for user_id in list(broadcast_users):
                try:
                    await self.bot.send_message(user_id, f"📢 **Announcement:**\n\n{message}")
                    success += 1
        ...
        @self.bot.on(events.NewMessage(pattern='/removebot'))
...
🔒 **Only the mentioned user can read your message!**
            """.format((await self.bot.get_me()).username, 
        ...
        @self.bot.on(events.NewMessage(pattern=r'/clone\s+(\S+)'))
        ...
        try:
            if self.initialize_bot():
                ...
                async def main():
                    ...
                    logger.info("🚀 Bot is now active and ready to receive messages!")
...
# Create bot instance
whisper_bot = WhisperBot()

def start_services():
    """Start both web server and bot"""
    # Start bot in a separate thread
    bot_thread = threading.Thread(target=whisper_bot.run_bot, daemon=True)
    bot_thread.start()
