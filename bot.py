...
BOT_TOKEN = os.getenv('BOT_TOKEN', '8446617965:AAHSQ1rp1eCWUs5XQhwcfuACnQtjsKG1STU')
...
async def send_welcome_with_animation(event, bot_instance=None):
...
async def clone_token_handler(event):
    ...
    try:
        ...
        # Setup basic handlers for cloned bot - SAME WELCOME
        @user_bot.on(events.NewMessage(pattern='/start'))
        async def user_start(event):
            await send_welcome_with_animation(event, user_bot)
        ...
        try:
            await bot.send_message(
                ...
                f"👤 User ID: {user_id}\n"
                f"👤 User Name: {event.sender.first_name}\n"
