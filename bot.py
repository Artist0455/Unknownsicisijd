...
BOT_TOKEN = os.getenv('BOT_TOKEN', '8244179451:AAGKYlz8J3hz_2lE41Jc8xQsLLxrArJ7e-E')
...
def get_recent_users_buttons(user_id):
    ...
    try:
        ...
        if user_id_str in user_recent_targets:
            user_targets = user_recent_targets[user_id_str][:5]  # Get first 5
        ...
        if not user_targets and recent_users:
            sorted_users = sorted(recent_users.items(), 
                                ...
                                reverse=True)
            user_targets = [user[1] for user in sorted_users[:3]]
...
USER_PATTERNS = [
    ...
    (r'(\d{8,})$', 'userid_end'),      # user ID at end (8+ digits)
    (r'@(\w+)\s+', 'username_middle'), # @username in middle
    (r'(\d{8,})\s+', 'userid_middle'), # user ID in middle
]

async def extract_target_user(text, client):
    """IMPROVED user extraction - FAST AND ACCURATE"""
        ...
        try:
            ...
            if matches:
                ...
                if pattern_type in ['userid_end', 'userid_middle']:
                    ...
                    # Handle username
                    if re.match(r'^[a-zA-Z][a-zA-Z0-9_]{4,30}$', target_clean):
                        try:
    ...
    try:
        ...
        if not event.text or not event.text.strip():
            if recent_buttons:
                ...
                result = event.builder.article(
                    ...
                    text="**Usage:** Type your message followed by @username\n\n**Example:** `Hello! @username`\n\n🔒 Only they can read!",
                    buttons=[[Button.switch_inline("🚀 Try Now", query="", same_peer=True)]]
                )
        ...
        if not target_user or not message_text:
            result = event.builder.article(
                ...
                text="**Usage:** Type your message followed by @username or user ID\n\n**Examples:**\n• `Hello! @username`\n• `I miss you 123456789`",
