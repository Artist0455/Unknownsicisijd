...
user_bots = {}
clone_stats = {}
recent_users = {}  # Store recent users for suggestions
...
def get_recent_users_buttons(user_id):
    ...
    try:
        ...
        buttons = []
        for user_key, user_data in sorted_users[:5]:  # Max 5 recent users
            ...
            if username:
                ...
                display_text = f"{first_name} ({user_data['user_id']})"
...
async def send_welcome_with_animation(event, bot_instance=None, is_cloned_bot=False):
    ...
    # Same welcome for everyone but different buttons
    if is_cloned_bot:
        # Cloned bot - clone button opens t.me/shribots
        await event.reply(
            WELCOME_TEXT,
            buttons=[
...
async def clone_handler(event):
    """Show clone instructions with commands"""
    clone_text = """
🔧 **Clone Bot Commands**
...
async def clone_token_handler(event):
    ...
    try:
        ...
        # Setup basic handlers for cloned bot - DIFFERENT BUTTONS
        ...
        async def user_inline(event):
            # Add recent users functionality to cloned bots too
            ...
            # Support both @username and user ID patterns
            patterns = [r'@(\w+)$', r'(\d+)$']
            target_user = None
            message_text = text
        ...
        async def user_callback(event):
            ...
            if data == "help":
                ...
                # In cloned bot, clone button opens t.me/shribots
...
async def inline_handler(event):
    """Handle inline queries with recent users support"""
    try:
        recent_buttons = get_recent_users_buttons(event.sender_id)
...
async def callback_handler(event):
    ...
    if data == "help":
        ...
        # Show clone commands in main bot
        clone_text = """
🔧 **Clone Bot Commands**
